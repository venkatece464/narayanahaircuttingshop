...
function initImageDeploymentFallbacks() {
    ...
    // Extra safety check for Vercel static asset serving
    setTimeout(() => {
        const logoImg = document.querySelector('.brand-logo-img');
        if (logoImg && (logoImg.naturalWidth === 0 || logoImg.clientWidth === 0)) {
            if (window.EMBEDDED_IMAGES && window.EMBEDDED_IMAGES.logo) {
                logoImg.src = window.EMBEDDED_IMAGES.logo;
            }
        }
    }, 150);
}
...
function triggerImageFallback(img) {
    if (!window.EMBEDDED_IMAGES) return;
    if (img.classList.contains('brand-logo-img') && window.EMBEDDED_IMAGES.logo) {
        img.src = window.EMBEDDED_IMAGES.logo;
    } else if (img.classList.contains('hero-img') && window.EMBEDDED_IMAGES.hero) {
